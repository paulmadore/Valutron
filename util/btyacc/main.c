...
static void usage (void)
{
    static const char * msg[] = {
        ...
        "  -L                    enable position processing, e.g., "
        "\"%locations\"",
        "  -o output_file        (default \"" OUTPUT_SUFFIX "\")",
        "  -p symbol_prefix      set symbol prefix (default \"yy\")",
        "  -P                    create a reentrant parser, e.g., "
        "\"%pure-parser\"",
        "  -r                    produce separate code and table files "
        "(y.code.c)",
        "  -s                    suppress #define's for quoted names in %token "
        "lines",
...
static void setflag (int ch)
{
    ...
    case 'V': printf ("%s - %s\n", myname, VERSION); exit (EXIT_SUCCESS);
