...
static bucket * default_destructor[3] = {0, 0, 0};

#define UNTYPED_DEFAULT 0
#define TYPED_DEFAULT 1
#define TYPE_SPECIFIED 2

static bucket * lookup_type_destructor (char * tag)
{
...
static char * copy_string (int quote)
{
 ...
 */
static int trim_blanks (char * buffer)
{
    if (*buffer != '\0')
    {
        char * d = buffer;
...
static void save_param (int k, char * buffer, int name, int type2)
{
    param *head, *p;
...
static char * cache_tag (char * tag, size_t len)
{
    ...
    for (i = 0; i < ntags; ++i)
    {
        if (strncmp (tag, tag_table[i], len) == 0 && tag_table[i][len] == NUL)
            return (tag_table[i]);
    }
    ...
    {
        tagmax += 16;
        tag_table = (tag_table ? TREALLOC (char *, tag_table, tagmax)
                               : TMALLOC (char *, tagmax));
        NO_SPACE (tag_table);
    }
...
static void read_declarations (void)
{
    ...
    for (;;)
    {
        ...
        case IDENT: copy_ident (); break;

        case TEXT: copy_text (); break;

        case UNION: copy_union (); break;

        case TOKEN:
        case LEFT:
        case RIGHT:
        case NONASSOC: declare_tokens (k); break;

        case EXPECT:
        case EXPECT_RR: declare_expect (k); break;

        case TYPE: declare_types (); break;

        case START: declare_start (); break;

        case PURE_PARSER: pure_parser = 1; break;

        case PARSE_PARAM:
        case LEX_PARAM: copy_param (k); break;

        case TOKEN_TABLE: token_table = 1; break;

#if defined(YYBTYACC)
        case LOCATIONS: locations = 1; break;
...
/* set immediately prior to where copy_args() could be called, and incremented
   by
...
static void advance_to_start (void)
{
    ...
    for (;;)
    {
        ...
        switch (keyword ())
        {
        case MARK: no_grammar ();

        case TEXT: copy_text (); break;

        case START: declare_start (); break;

        default: syntax_error (lineno, line, s_cptr);
        }
    }
    ...
    for (;;)
    {
        ...
        if (c == '<')
        {
            ...
            { /* "no per-symbol or per-type" default destructor */
                ...
                {
                    /* postpone re-processing destructor $$s until end of
                     * grammar spec */
        ...
        { /* "symbol" destructor */
            ...
            {
                /* postpone re-processing destructor $$s until end of grammar
                 * spec */
                bp->destructor = TMALLOC (char, strlen (code_text) + 1);
                NO_SPACE (bp->destructor);
                strcpy (bp->destructor, code_text);
...
#if defined(YYBTYACC)
static void finalize_destructors (void)
{
    int i;
    bucket * bp;
    char * tag;
    ...
    for (i = 2; i < nsyms; ++i)
    {
        ...
        if (symbol_destructor[i] == NULL)
        {
            ...
            { /* use <> destructor, if there is one */
                ...
                {
                    symbol_destructor[i] =
                        TMALLOC (char, strlen (bp->destructor) + 1);
