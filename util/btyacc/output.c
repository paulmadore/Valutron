...
static void putl_code (FILE * fp, const char * s)
{
    if (fp == code_file)
        ++outline;
    fputs (s, fp);
...
static void puts_code (FILE * fp, const char * s) { fputs (s, fp); }

static void puts_param_types (FILE * fp, param * list, int more)
{
    param * p;
...
static void puts_param_names (FILE * fp, param * list, int more)
{
    param * p;

    for (p = list; p; p = p->next)
    {
        fprintf (fp, "%s%s", p->name, ((more || p->next) ? ", " : ""));
    }
}
...
static void output_int (int value) { fprintf (output_file, "%5d,", value); }
...
static void start_int_table (const char * name, int value)
{
    ...
    if (need < 6)
        need = 6;
    fprintf (output_file, "%sconst YYINT %s%s[] = {%*d,", StaticOrR,
             symbol_prefix, name, need, value);
}

static void start_str_table (const char * name)
{
    fprintf (output_file, "%sconst char *const %s%s[] = {", StaticOrR,
             symbol_prefix, name);
    output_newline ();
}
...
#if defined(YYBTYACC)
static void output_accessing_symbols (void)
{
    int i, j;
    int * translate;
...
static Value_t find_conflict_base (int cbase)
{
    int i, j;
...
#if defined(YYBTYACC)
static void output_ctable (void)
{
    int i;
    int j;
...
#if defined(YYBTYACC)
static void output_backtracking_parser (FILE * fp)
{
    ...
    if (backtrack)
    {
        ...
        putl_code (fp, "#define YYDEBUGSTR (yytrial ? YYPREFIX "
                       "\"debug(trial)\" : YYPREFIX \"debug\")\n");
    }
    else
#endif
    {
...
#if defined(YYBTYACC)
static void output_ltype (FILE * fp)
{
    putc_code (fp, '\n');
...
#if defined(YYBTYACC)
static void output_yydestruct_decl (FILE * fp)
{
    putc_code (fp, '\n');
    putl_code (fp, "#ifndef YYDESTRUCT_DECL\n");

    puts_code (fp, "#define YYDESTRUCT_DECL() "
...
static void output_yydestruct_impl (void)
{
    int i;
...
static void output_externs (FILE * fp, const char * const section[])
{
    int i;
    const char * s;
