...
#if defined(YYBTYACC)
#define MaySuppress(p)                                                         \
    ((backtrack ? ((p)->suppressed <= 1) : (p)->suppressed == 0))
/* suppress the preferred action => enable backtracking */
#define StartBacktrack(p)                                                      \
    if (backtrack && (p) != NULL && NotSuppressed (p))                         \
    (p)->suppressed = 1
#else
...
static void remove_conflicts (void)
{
    ...
    for (i = 0; i < nstates; i++)
    {
        ...
        for (p = parser[i]; p; p = p->next)
        {
            if (p->symbol != symbol)
            {
                /* the first parse action for each symbol is the preferred
                 * action */
                pref   = p;
                symbol = p->symbol;
            }
            /* following conditions handle multiple, i.e., conflicting, parse
             * actions */
